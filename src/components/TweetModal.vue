    ...
    <div class="tweet__modal">
      ...
      >
        ...
        <textarea
          ...
          placeholder="有什麼新鮮事 ?"
        ></textarea>
        <span class="tweet__modal__content__length">{{ text.length }}/140</span>
        <span v-if="text.length > 140" class="tweet__modal__content__notice"
          >字數不可超過 140 字</span
        >
        <span v-if="!text.length" class="tweet__modal__content__notice"
          >內容不可空白</span
        >
...
.tweet__modal__close__button {
  padding: 0;
  vertical-align: top;
}

.tweet__modal__close__button__img {
  width: 15px;
  height: 15px;
}

.tweet__modal__content {
  width: 100%;
  height: 245px;
  position: relative;
  padding: 16px;
...
.tweet__modal__content__textarea {
  resize: none;
  height: 160px;
  width: 470px;
  font-size: 16px;
...
.tweet__modal__content__textarea::placeholder {
  font-weight: 500;
}

.tweet__modal__content__button {
  position: absolute;
  bottom: 0;
  right: 0;
  color: white;
